...
For this task, I ran the command ssh cs15lwi23ate@ieng6-202.ucsd.edu and the account logged in immediately. This is because I had generated an SSH key for ieng6 and now I do not need to put in a password to log in everytime. 
...
For this task, since I had already run these tests the last time I was doing the tasks, I had the commands javac -cp .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar *.java and java -cp .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar org.junit.runner.JUnitCore ListExamplesTests much higher up in my search history already (8 commands up). I accessed them with these keys pressed: < up > < up > < up > < up > < up > < up > < up > < up > < enter > and  < up > < up > < up > < up > < up > < up > < up > < up > < enter > respectively.
...
For this task, I opened up the ListExamples.java file through vim (as I was shown by the tutor) and corrected the code by pressing "i" to insert and pressing < escape > and then ":w" to write into the code and then ":q" to quit vim. 
...
In the vim after I pressed "i" and edited the code: 
...
Now in order to access the javac -cp .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar *.java command, I had to go up 3 lines, so I pressed these keys and accessed it: < up > < up > < up > < enter>

Then, to access the java -cp .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar org.junit.runner.JUnitCore ListExamplesTests command, I had to also go up 3 lines, so I pressed these keys to access it: < up > < up > < up > < enter>
...
Finally to commit and push into my Github account, I needed to use the git add ListExamples.java command, git commit -m "updated file", and git push that I had used already, however, as they were many commands before and these commands were short anyway, instead of going up in my search history, I just manually inputted these commands. 
